...
class TipoUsuarioService
{
    ...
    {
        return TipoUsuario::orderBy('id_tipo_usuario', 'asc')->get();
    }

    public function paginate($perPage = 5, $buscar = null)
    {
        return TipoUsuario::when($buscar, function ($query, $buscar) {
    ...
    {
        $registros = TipoUsuario::when($buscar, function ($query, $buscar) {
                return $query->where('nombre_tipo', 'like', "%{$buscar}%");
            })
            ->orderBy('id_tipo_usuario', 'asc')
            ->get();
        ...
        foreach ($registros as $reg) {
            $sheet->setCellValue('A' . $row, $reg->id_tipo_usuario);
        ...
        return new StreamedResponse(function () use ($writer) {
            ...
            "Content-Disposition" => "attachment; filename=tipo_usuario.xlsx",
        ]);
    }
}
