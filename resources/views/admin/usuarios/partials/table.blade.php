                        ...
                        <td class="ps-4">
                            <div class="d-flex align-items-center gap-3">
                                <div class="rounded-circle overflow-hidden shadow-sm" style="width: 40px; height: 40px;">
                                    @if($u->foto_usuario)
                                        <img src="{{ asset('storage/' . $u->foto_usuario) }}"
                                            style="width: 100%; height: 100%; object-fit: cover;" alt="Foto">
                                    @else
                                        <div
                                            class="bg-primary bg-opacity-10 p-2 text-primary d-flex align-items-center justify-content-center h-100">
                                            <span class="material-symbols-rounded">person</span>
                                        </div>
                                    @endif
                                </div>
                                <div>
                                    <span class="fw-bold d-block text-dark">{{ $u->primer_nombre }}
                                        {{ $u->primer_apellido }}</span>
                        ...
                        <td>
                            ...
                            @if(isset($docs_licencia[$u->doc_usuario]))
                                ...
                                @if(!$lic->isVigente())
                                    <div class="mt-1">
                                        <span
                                            class="badge bg-danger bg-opacity-10 text-danger rounded-pill px-2 d-inline-flex align-items-center gap-1"
                                            style="font-size: 0.65rem;">
                                            <span class="material-symbols-rounded" style="font-size: 0.8rem;">error</span> Licencia
                                            Vencida
                                        </span>
                                    </div>
                                @endif
                        ...
                        <td class="text-end pe-4">
                            <div class="d-flex justify-content-end gap-3">
                                <a href="#" class="text-info text-decoration-none d-flex align-items-center"
                                    title="Ver detalles" data-doc="{{ $u->doc_usuario }}"
                                    data-primer-nombre="{{ $u->primer_nombre }}"
                                    data-segundo-nombre="{{ $u->segundo_nombre }}"
                                    data-primer-apellido="{{ $u->primer_apellido }}"
                                    data-segundo-apellido="{{ $u->segundo_apellido }}" data-correo="{{ $u->correo }}"
                                    data-telefono="{{ $u->telefono }}" data-rol="{{ $u->nombre_tipo }}"
                                    data-estado="{{ $u->nombre_estado }}" data-ciudad="{{ $u->nombre_city }}"
                                ...
                                <a href="#" class="text-primary text-decoration-none d-flex align-items-center"
                                    title="Editar usuario" data-doc="{{ $u->doc_usuario }}"
                                    data-primer-nombre="{{ $u->primer_nombre }}"
                                    data-segundo-nombre="{{ $u->segundo_nombre }}"
                                    data-primer-apellido="{{ $u->primer_apellido }}"
                                    data-segundo-apellido="{{ $u->segundo_apellido }}" data-correo="{{ $u->correo }}"
                                    data-telefono="{{ $u->telefono }}" data-rol="{{ $u->id_tipo_usuario }}"
                                    data-estado_id="{{ $u->id_estado }}" data-foto="{{ $u->foto_usuario }}"
